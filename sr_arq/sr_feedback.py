...
class SRFeedbackMode(Enum):
    ...
    - E2E_TIMEOUT: reproduces the external uncoded SR-ARQ baseline. Intermediate
      nodes are best-effort forwarders (identical relays to E2E_FORWARD_ONLY: no
      per-hop retransmit/feedback), so a packet is delivered only if it survives
      all H hops. Unlike E2E_FORWARD_ONLY the receiver sends NO NACKs, only an
      end-to-end ACK per arrival; the source recovers losses purely by a
      retransmission TIMER (resend an outstanding seq once it has gone at least one
      end-to-end RTT without an ACK). Per-path rate is the product of the per-hop
      rates, like E2E_FORWARD_ONLY.
    """
    ...
    def uses_forward_only_relays(self) -> bool:
        """True when intermediate nodes are best-effort forwarders (no per-hop
        SR-ARQ): the E2E_FORWARD_ONLY and E2E_TIMEOUT modes."""
        return self in (SRFeedbackMode.E2E_FORWARD_ONLY, SRFeedbackMode.E2E_TIMEOUT)
